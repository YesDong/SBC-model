function [X,F] = SBC(a1,a2,a3,r1,r2,r3,del,o,T)
...
% r3: denoting the fusion parameter of [3]-order
...
% F: n*3*T, denoting the increment of each individual's opinion value in each iteration
%%%%Example%%%%
% close all
% clear all
% clc
...
while t<=T
    ...
    X(:,t)=x;
    F1=-diag(D1.*K1*a1{t}(:,:));
    F2=-permute(sum(sum(D2.*K2.*a2{t}(:,:,:),1),2),[3,2,1]);
    F3=-permute(sum(sum(sum(D3.*K3.*a3{t}(:,:,:,:),1),2),3),[4,3,2,1]);
